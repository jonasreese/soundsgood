...
public class OSCByteArrayToJavaConverter {
    ...
    /**
     * Read a MIDI message from the byte stream.
     * @return a MIDI message
     */
    private MidiMessage readMidiMessage() {
        ShortMessage message = new ShortMessage();
        try {
            int portId = Bitdata.signedToUnsigned(bytes[streamPosition++]); // port ID
            if (portId > 0) {} // avoid 'unused' compiler warning
            int status = Bitdata.signedToUnsigned(bytes[streamPosition++]);
            int data1 = Bitdata.signedToUnsigned(bytes[streamPosition++]);
            int data2 = Bitdata.signedToUnsigned(bytes[streamPosition++]);
